...
| `fact_region_product_monthly.csv` | one **region × product × month** total | a quick pre-aggregated trend if you prefer |
...
> **"Calling doctors works — and a specific 99-doctor 'High-potential growth' segment is where extra
> calls will return the most prescriptions."** Every chart should support that story.
...
4. **Build these 5 sheets** (each on a new sheet — the tab with the small chart icon at the bottom):
   ...
   - **KPI card — Total scripts.** Drag `script_count` to **Text**. It defaults to SUM — that's what we
     want. (Optional: format with thousands separators via the measure's number format.)
   - **Bar — Scripts by region.** Drag `region` (from `dim_region`) to **Columns** and `script_count`
     to **Rows**. Sort descending with the toolbar sort button. **Headline:** Southeast leads.
   - **Bar — Scripts by product.** Drag `product` (from `dim_product`) to **Columns**, `script_count`
     to **Rows**. **Headline:** Cardiplex is the flagship.
   - **Scatter — calls vs scripts (with trend line).** Use `dim_hcp`: drag `hcp_total_calls` to
     **Columns** and `hcp_total_scripts` to **Rows**; drag `hcp_id` to **Detail** so each dot is one
     doctor. Open the **Analytics** tab (top-left) and drag **Trend Line** onto the chart. **Headline:**
     the upward line is the proof that calling works.
...
5. **Assemble a dashboard:** click the **New Dashboard** icon (bottom). Put the **Total scripts** KPI
   card top-left, the two region/product bars across the top, and the scatter + segment chart below.
...
7. **Calculated field you'll want — Scripts per call:** Analysis → Create Calculated Field, name it
   `Scripts per Call`, formula:
   ```
   SUM([Hcp Total Scripts]) / SUM([Hcp Total Calls])
   ```
   (Use the `dim_hcp` fields.) Drop it on a bar by `segment_name` to show which segment is most responsive.
...
2. **Get data:** **Home → Get data → Text/CSV** → load all six CSVs (one at a time). In **Transform
   data**, confirm `script_month` is a *Date* type and `script_count` is a *Whole number*.
...
6. **Add a slicer:** drop a **Slicer** visual and put `dim_region[region]` (or `dim_product[product]`)
   in it so viewers can filter.
...
│   scripts    │  Bar: Scripts by product (Cardiplex flagship) │
│ KPI: #Doctors│                                               │
├──────────────┴───────────────────────────────────────────────┤
│ Scatter: calls vs scripts (trend line) │ Bar: doctors by      │
│   — "calling works"                     │   segment + scripts/ │
│                                         │   call (the target)  │
└─────────────────────────────────────────┴──────────────────────┘
   Slicer:  [ region ▼ ]
```

## One clear headline insight to put on the dashboard title
> **"More calls → more scripts. Focus the next calls on the 99-doctor High-potential Growth segment."**
